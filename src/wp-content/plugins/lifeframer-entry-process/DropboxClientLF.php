...
require_once(dirname(__FILE__)."/OAuthSimpleLF.php");

class DropboxClientLF {
	...
	function __construct ($app_params, $locale = "en"){
		...
		if(empty($app_params['app_key']))
			throw new DropboxExceptionLF("App Key is empty!");
	...
	public function GetRequestToken($get_new_token=false)
	{
		...
		if(empty($rt) || empty($rt['oauth_token']))
			throw new DropboxExceptionLF('Could not get request token!');
	...
	public function BuildAuthorizeUrl($return_url)
	{
		...
		if(empty($rt) || empty($rt['t'])) throw new DropboxExceptionLF('Request Token Invalid ('.print_r($rt,true).').');
	 ...
	 *
	 * @access public
	 * @param array $request_token Optional. The previously retrieved request token. This parameter can only be skipped if the DropboxClientLF object has been (de)serialized.
	...
	public function GetAccessToken($request_token = null)
	{
		...
		if(empty($request_token)) throw new DropboxExceptionLF('Request token required!');

		$at = $this->authCall("oauth/access_token", $request_token);
		if(empty($at))
			throw new DropboxExceptionLF(sprintf('Could not get access token! (request token: %s)', $request_token['t']));
	...
	{
		if(empty($token['t']) || empty($token['s'])) throw new DropboxExceptionLF('Passed invalid access token.');
	...
	public function DownloadFile($dropbox_file, $dest_path='', $rev=null, $progress_changed_callback = null)
	{
		...
		if($fh === false) {
			@fclose($rh);
			throw new DropboxExceptionLF("Could not create file $dest_path !");
		}
		...
		if($this->useCurl) {
			...
			if($rh === false)
				throw new DropboxExceptionLF("HTTP request to $url failed!");
			...
			while (!feof($rh)) {
				if(($s=fwrite($fh, fread($rh, self::BUFFER_SIZE))) === false) {
					@fclose($rh);
					@fclose($fh);
					throw new DropboxExceptionLF("Writing to file $dest_path failed!'");
				}
				$bytes_loaded += $s;
		...
		if($meta->bytes != $bytes_loaded)
			throw new DropboxExceptionLF("Download size mismatch! (header:{$meta->bytes} vs actual:{$bytes_loaded}; curl:{$this->useCurl})");
	...
	public function UploadFile($src_file, $dropbox_path='', $overwrite=true, $parent_rev=null)
	{
		...
		if($file_size > self::MAX_UPLOAD_CHUNK_SIZE)
		{
			...
			while(!feof($fh)) {
				...
				if(empty($upload_id)) {
					...
					if(empty($upload_id)) throw new DropboxExceptionLF("Upload ID empty!");
				}
			}
		...
		if($this->useCurl) {
			...
			if(strlen($content) == 0)
				throw new DropboxExceptionLF("Could not read file $src_file or file is empty!");
	...
	function getFileTree($path="", $include_deleted = false, $max_depth = 0, $depth=0)
	{
		...
		if(!empty($dir->error)) throw new DropboxExceptionLF($dir->error);
	...
	static function &execCurlAndClose($ch, &$out_response_headers = null)
	{
		...
		curl_close($ch);
		if($err_no || $res === false) {
			throw new DropboxExceptionLF("cURL-Error ($err_no): $err_str");
		}
	...
	private function createRequestContext($url, $method, &$content=null, $oauth_token=-1)
	{
		...
		$oauth = new OAuthSimpleLF($this->consumerToken['t'],$this->consumerToken['s']);
	...
	{
		if(!empty($resp->error))
			throw new DropboxExceptionLF($resp->error);
		return $resp;
	}
...
//			throw new DropboxExceptionLF();
	...
	private static function getMetaFromHeaders(&$header_array, $throw_on_error=false)
	{
		...
		if($throw_on_error && (empty($obj)||!is_object($obj)))
			throw new DropboxExceptionLF("Could not retrieve meta data from header data: ".print_r($header_array,true));
		if($throw_on_error)
...
class DropboxExceptionLF extends Exception {
