...
class OAuthSimpleLF {
    ...
    public function setURL ($path)
	{
        if (empty($path))
		{
            throw new OAuthSimpleExceptionLF('No path specified for OAuthSimpleLF.setURL');
		}
        $this->_path=$path;
    ...
    public function setAction ($action) 
    {
		...
		{
            throw new OAuthSimpleExceptionLF('Invalid action specified for OAuthSimpleLF.setAction');
		}
        $this->_action = $action;
    ...
    public function signatures ($signatures) 
    {
		...
		{
            throw new OAuthSimpleExceptionLF('Must pass dictionary array to OAuthSimpleLF.signatures');
		}
        if (!empty($signatures))
        {
		...
		{
            throw new OAuthSimpleExceptionLF('Missing required consumer_key in OAuthSimpleLF.signatures');
        }
        if (empty($this->_secrets['shared_secret']))
		{
            throw new OAuthSimpleExceptionLF('Missing requires shared_secret in OAuthSimpleLF.signatures');
		}
        if (!empty($this->_secrets['oauth_token']) && empty($this->_secrets['oauth_secret']))
		{
            throw new OAuthSimpleExceptionLF('Missing oauth_secret for supplied oauth_token in OAuthSimpleLF.signatures');
		}
    ...
    public function setSignatureMethod ($method="")
	{
        ...
        switch($method)
        {
            ...
            case 'HMAC-SHA1':
				...
				break;
            default:
                throw new OAuthSimpleExceptionLF ("Unknown signing method $method specified for OAuthSimpleLF.setSignatureMethod");
				break;
        }
    ...
    */
    ...
    private static function _oauthEscape($string) 
    {
        ...
        if (is_array($string)) {
            throw new OAuthSimpleExceptionLF('Array passed to _oauthEscape');
		}
    ...
    private function _getApiKey() 
    {
        if (empty($this->_secrets['consumer_key']))
        {
            throw new OAuthSimpleExceptionLF('No consumer_key set for OAuthSimpleLF');
        }
    ...
    private function _getAccessToken() 
    {
		...
		{
            throw new OAuthSimpleExceptionLF('No access token (oauth_token) set for OAuthSimpleLF.');
		}
    ...
    private function _generateSignature () 
    {
        ...
        switch($this->_parameters['oauth_signature_method'])
        {
            ...
            default:
                throw new OAuthSimpleExceptionLF('Unknown signature method for OAuthSimpleLF');
				break;
        }
    }
}

class OAuthSimpleExceptionLF extends Exception {
