...
global $ultimatemember;
    if (isset($_POST['jatts'])) { $jatts = $_POST['jatts']; $jatts = stripslashes($jatts); $jatts = json_decode($jatts); $atts = $jatts; }
...
$users = explode(',', $users);
$akm = $pieces . " " . $limit;
$users = array_slice($users, $pieces, $limit);
...
if (!empty($user_query->results)) {
    foreach ( $user_query->results as $user ) {
        ...
        //             $content .= "<a href='" . $homeurl . "/build-your-profile' title='titlu'>";
        //                 $content .= "<img src='http://via.placeholder.com/450x300' alt=''>";
        //             $content .= "</a>";

        //             $content .= "<div class='um-member-card no-photo'>";

        //                 $content .= "<div class='um-member-name a'>";
        //                     $content .= "<a href='" . $homeurl . "/build-your-profile' title=''>Build your Profile</a>";
        //                 $content .= "</div>";
        //             $content .= "</div>";
        //         $content .= "</div>";
        //     $content .= "</div>";
        ...
        // else {
            ...
            // echo '<pre>'.print_r($permalink,1).'</pre>';
...
function swph_ultimatemember($atts = array(), $content = ""){
    $jatts = json_encode($atts);
    $output = swph_get_members($atts);
    $userno = explode(",", $atts['users']);
    $userno = count($userno);
    $times = $userno / $atts['limit'];
    $times = ceil($times);
    $output = "<div class='ns-collection um-form'><div class='row'>" . $output;
    $output .= "</div>";
    $output .= "<div class='butrow row text-center'><button id='loadmore' data-times='" . $times . "' data-piece='1' class='fusion-button button-flat fusion-button-round button-large button-custom home_cta'>Load more...</button><span id='jatts' style='display: none;'>" . $jatts . "</span></div>";
    $output .= "</div>";
...
function website_custom_validation( $args ) {
    ...
    if ( isset( $args['social_website'] ) && ($args['social_website']!='') && ( (!filter_var($args['social_website'], FILTER_VALIDATE_URL)) || (!preg_match('/^(http|https):\\/\\/[a-z0-9_]+([\\-\\.]{1}[a-z_0-9]+)*\\.[_a-z]{2,5}'.'((:[0-9]{1,5})?\\/.*)?$/i',$args['social_website'],$matches))) ) {
...
if(isset($_GET['test']) && ($_GET['test'] == 'test_img') ){
    // add_action('template_redirect','test_img');
}
